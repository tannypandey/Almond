...
// Authorization code is still left
...
const telegram;// = require('telegram-node-client').Telegram;
...
const CONSUMER_KEY = process.env['telegram_CONSUMER_KEY'] || 'VZRViA2T4qy7CBZjU5juPumZN';
// telegram uses OAuth 1.0, so this needs to be here...
const CONSUMER_SECRET = process.env['telegram_CONSUMER_SECRET'] || rot13('hsTCqM6neIt3hqum6zvnDCIqQkUuyWtSjKBoqZFONvzVXfb7OJ');

function makeTelegramApi(engine, accessToken, accessTokenSecret) {
    var origin = engine.platform.getOrigin();
    return new Telegram({
        consumerKey: CONSUMER_KEY,
        consumerSecret: CONSUMER_SECRET,
        callBackUrl: origin + '/devices/oauth2/callback/com.telegram',
...
function runOAuthStep1(engine) {
    let telegram = makeTelegramApi(engine);

    return new Promise((callback, errback) => {
        telegram.oauth.getOAuthRequestToken((error, oauth_token, oauth_token_secret, query) => {
            if (error)
                errback(error);
            else
        ...
        const url = Url.parse('https://api.telegram.com/oauth/authorize');
        url.query = result.query;
        url.query['oauth_token'] = result.token;
        url.query['oauth_token_secret'] = result.tokenSecret;
        return [Url.format(url), { 'telegram-token': result.token,
                                   'telegram-token-secret': result.tokenSecret }];
    });
}
...
function runOAuthStep2(engine, req) {
    ...
    return new Promise((callback, errback) => {
        const token = req.session['telegram-token'];
        const tokenSecret = req.session['telegram-token-secret'];
        const verifier = req.query['oauth_verifier'];

        telegram.oauth.getOAuthAccessToken(token, tokenSecret, verifier, (error, oauth_access_token, oauth_access_token_secret, results) => {
            if (error)
                errback(error);
            else
        ...
        telegram = maketelegramApi(engine, result.accessToken, result.accessTokenSecret);
        return new Promise((callback, errback) => {
            telegram.getCustomApiCall('/account/verify_credentials.json', {}, errback, callback);
        });
    }).then((result) => {
        result = JSON.parse(result);
        return engine.devices.loadOneDevice({ kind: 'com.telegram',
                                              accessToken: telegram.accessToken,
                                              accessTokenSecret: telegram.accessTokenSecret,
...
module.exports = class TelegramAccountDevice extends Tp.BaseDevice {
    ...
    _pollDirectMessages(since_id) {
        return new Promise((callback, errback) => {
            if (since_id !== undefined)
                this._telegram.getCustomApiCall('/direct_messages/events/list.json', { since_id: since_id, count: 20 }, errback, callback);
            else
                this._telegram.getCustomApiCall('/direct_messages/events/list.json', { count: 20 }, errback, callback);
    ...
    do_send_direct_message({ to, message }) {
        return new Promise((callback, errback) => {
            this._pollUserId(String(to)).then((user_id) => {
                ...
                this._telegram.oauth.post(this._telegram.baseUrl + '/direct_messages/events/new.json',
                    this._telegram.accessToken,
                    this._telegram.accessTokenSecret,
                    JSON.stringify(data),
                    "application/json",
                    (err, body, response) => {
